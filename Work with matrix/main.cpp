...
public:
	...
	friend const matrix operator -(matrix const& other) {
		...
		temp.matr = new double[matrdim];
		for (int i = 0; i < matrdim; i++) temp.matr[i] = other.matr[i] * -1;
	...
	double det(double* matr, int dim) {
		...
		if (dim == 1) {
			...
			d = matr[0] * matr[3] - matr[1] * matr[2];
		}
		else {
			for (int k = 0; k < dim - 1; k++) {
				for (int i = k + 1; i < dim; i++) {
					tmp = -temp[k + dim * i] / temp[k + dim * k];
					for (int j = 0; j < dim; j++) {
						temp[j + dim * i] += temp[j + dim * k] * tmp;
					}
				}
			}
			for (int i = 0; i < dim; i++) {
	...
	double* rotating(matrix const& other, int x, int y) {
		matrix temp;
		...
		for (int i = 0; i < other.dim; i++) {
			if (i != x) {
				for (int j = 0; j < other.dim; j++) {
					if (j != y) {
						temp.matr[pos] = matr[j + other.dim * i];
						pos++;
					}
				}
	...
	matrix rev() {
		...
		for (int i = 0; i < dim; i++) {
			for (int j = 0; j < dim; j++) {
				temp.matr[j + i * dim] = det(rotating(*this, i, j), dim - 1);
			}
		}
		for (int i = 0; i < dim; i++) {
			for (int j = 0; j < dim; j++) {
...
int main() {
	...
	// an example, so we won't get 2 same libraries
	Aarr[8] = 8;
	matrix a(3, Aarr);
	matrix b(3, Barr);
